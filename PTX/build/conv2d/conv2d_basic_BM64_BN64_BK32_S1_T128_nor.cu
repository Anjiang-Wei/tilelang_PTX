...
extern "C" __global__ void __launch_bounds__(128, 1) main_kernel(half_t* __restrict__ data, half_t* __restrict__ out_flat, half_t* __restrict__ w_flat) {
  ...
  for (int i_5 = 0; i_5 < 4; ++i_5) {
    if (((((((int)blockIdx.y) * 16) + (i_5 * 4)) + (((int)threadIdx.x) >> 5)) < 75625) && (((((int)blockIdx.x) * 2) + ((((int)threadIdx.x) & 7) >> 2)) < 3)) {
      *(uint4*)(out_flat + (((((((int)blockIdx.y) * 6144) + (i_5 * 1536)) + ((((int)threadIdx.x) >> 3) * 96)) + (((int)blockIdx.x) * 64)) + ((((int)threadIdx.x) & 7) * 8))) = *(uint4*)(((half_t*)buf_dyn_shmem) + ((i_5 * 1024) + (((int)threadIdx.x) * 8)));
    }
  }
}
