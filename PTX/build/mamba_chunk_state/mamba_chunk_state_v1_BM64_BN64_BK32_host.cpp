...
class Module:
    ...
    def main(args: T.handle, arg_type_ids: T.handle("int32"), num_args: T.int32, out_ret_value: T.handle("void"), out_ret_tcode: T.handle("int32"), resource_handle: T.handle) -> T.int32:
        ...
        if not T.isnullptr(main_A_handle_strides):
            assert 1 == T.Cast("int32", main_A_handle_strides_1[1]) and 1024 == T.Cast("int32", main_A_handle_strides_1[0]), "main.A_handle.strides: expected to be compact array"
            T.evaluate(0)
        ...
        if not T.isnullptr(main_B_handle_strides):
            assert 1 == T.Cast("int32", main_B_handle_strides_1[1]) and 1024 == T.Cast("int32", main_B_handle_strides_1[0]), "main.B_handle.strides: expected to be compact array"
            T.evaluate(0)
        ...
        if not T.isnullptr(main_C_handle_strides):
            assert 1 == T.Cast("int32", main_C_handle_strides_1[1]) and 1024 == T.Cast("int32", main_C_handle_strides_1[0]), "main.C_handle.strides: expected to be compact array"
            T.evaluate(0)
        ...
        C_1 = T.decl_buffer((1024, 1024), "float16", data=C)
        ...
        with T.attr(0, "compute_scope", "main_compute_"):
            T.call_packed("main_kernel", A, B, C, 16, 16, 128, 1, 1, 8192)
        return 0